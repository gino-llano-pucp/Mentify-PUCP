...
const TableListCancelado = ({ estado, items}) => {
      ...
      <TableBody emptyContent={"No hay citas proximas encontradas."} items={items}>
        {((cita) => (
          <TableRow key={cita.idCita}>
            ...
            <TableCell>
                  {cita.alumnos.length > 1 ? (
                      <AvatarGroupComponent studentInfos={cita.alumnos} />
                    ) : (
                      <SingleUserComponent student={cita.alumnos[0]} />
                    )}
            </TableCell>
            <TableCell>
...
export default TableListCancelado;
