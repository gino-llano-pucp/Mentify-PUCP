...
const roleOptions = [
  ...
  { name: "Alumno", uid: "Alumno" }
];

const INITIAL_VISIBLE_COLUMNS = ['name', 'code', 'roles', 'programa'];

export default function ListadoUsuarios({ users, totalUsers, onSelect, onClose, currentPage, pages, onPageChange, setRowsPerPage, setRoleFilter, roleFilter, onSearchChange, filtro, onClear }) {
  ...
  useEffect(() => {
    ...
    if (roleFilter.size > 1) {
      setButtonLabel('Tutor');
    } else if (roleFilter.size === 1) {
      const selectedRole = roleOptions.find(option => roleFilter.has(option.uid));
      setButtonLabel(selectedRole ? selectedRole.name : 'Estado');
    } else {
  ...
  const renderCell = useCallback((user, columnKey) => {
    ...
    switch (columnKey) {
      ...
      case 'roles':
        const rolesToShow = user.roles.includes('Tutor') && user.roles.includes('Alumno') ? 'Tutor y Alumno' : user.roles.includes('Tutor') ? 'Tutor' : 'Alumno';
        return <span>{rolesToShow}</span>;
      case 'programa':
      ...
      <div className='flex flex-col gap-4'>
        <div className='flex items-end justify-between gap-3'>
          <Input
            ...
            placeholder='Ingresa el código, nombre o correo del alumno o tutor'
          ...
          <Dropdown className='min-w-32'>
            <DropdownTrigger>
              <Button 
                ...
                variant="flat"
                className='flex justify-between text-gray-700 min-w-28 w-28'
              >
                {buttonLabel}
              </Button>
            </DropdownTrigger>
            <DropdownMenu
              className='box-border w-28 min-w-28'
        ...
        <div className='flex items-center justify-between'>
          <span className='text-default-400 text-small'>Total {totalUsers} alumnos y tutores.</span>
  ...
  return (
    ...
    >
      ...
      <TableBody emptyContent={'No hay usuarios por listar.'} items={sortedItems}>
        {(item) => (
